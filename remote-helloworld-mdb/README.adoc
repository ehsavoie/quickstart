...
. Review the `configure-remote-broker.cli` file in the root of this quickstart directory. This script adds the `test` queue to the `messaging` subsystem in the server configuration file.
...
This script removes the remote broker connection from the `messaging` subsystem and restore the `default` internal broker. You should see the following result when you run the script.
